...
export function NewBlogClient({ lang, categories, authorName, authorEmail }: NewBlogClientProps) {
  ...
  useEffect(() => {
    if (formData.content && formData.content.length > 0 && !tipTapContent) {
      ...
      if (hasContent) {
        const tipTapJSON = contentBlocksToTipTap(formData.content)
        setTipTapContent(tipTapJSON)
      }
    } else if (!tipTapContent) {
      ...
      >
        <div className="container mx-auto px-6 py-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-[#2c2b2b]">Create New Blog Post</h1>
              <p className="text-[#2c2b2b]/70 mt-1">Add a new blog post to your website</p>
            </div>
          </div>
        </div>
      </motion.header>
